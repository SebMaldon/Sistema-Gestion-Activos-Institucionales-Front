...
export default function ProveedorModal({ onClose, onSuccess, proveedor = null }) {
 ...
 const labelCls = 'block text-xs font-bold text-gray-600 dark:text-gray-400 mb-1.5';
 ...
 <div className="w-8 h-8 rounded-lg bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 dark:text-green-300 flex items-center justify-center border border-green-100 dark:border-green-800/50">
 <Building size={16} />
 </div>
 <h2 className="text-base font-bold text-gray-900 dark:text-gray-100 ">{isEdit ? 'Editar Proveedor' : 'Registrar Proveedor'}</h2>
 </div>
 <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-400 transition-colors">
 <X size={18} />
 </button>
 </div>
 ...
 <label className="text-sm font-bold text-gray-800 dark:text-gray-200 ">Contactos del Proveedor</label>
 <button 
 type="button" 
 onClick={handleAddContacto}
 className="text-xs flex items-center gap-1 text-green-700 dark:text-green-400 dark:text-green-300 bg-green-50 dark:bg-green-900/20 px-2.5 py-1.5 rounded-lg hover:bg-green-100 transition-colors font-semibold border border-green-200 dark:border-green-800/50 dark:border-green-800/50"
 >
 ...
 {contactos.length === 0 ? (
 <p className="text-xs text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-900 p-4 rounded-xl border border-gray-100 dark:border-gray-800 text-center border-dashed">
 Aún no hay contactos registrados.
 </p>
 ) : (
 <div className="space-y-3">
 {contactos.map((c, i) => (
 <div key={i} className="flex gap-2 items-start bg-gray-50 dark:bg-gray-900 p-2 rounded-xl border border-gray-100 dark:border-gray-800 ">
 <select
 value={c.tipo_contacto}
 onChange={e => handleUpdateContacto(i, 'tipo_contacto', e.target.value)}
 className="px-2.5 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg outline-none focus:ring-2 focus:ring-green-500 bg-white dark:bg-gray-800 min-w-[120px]"
 >
 ...
 className="p-2 text-red-500 hover:bg-red-50 rounded-lg transition-colors border border-transparent hover:border-red-100 dark:border-red-800/50"
 ...
 {/* Footer Actions */}
 <div className="p-6 pt-0 mt-auto shrink-0 bg-white dark:bg-gray-800 border-t border-gray-50 dark:border-gray-800 flex gap-3 pt-4">
 <button
 type="button"
 onClick={onClose}
