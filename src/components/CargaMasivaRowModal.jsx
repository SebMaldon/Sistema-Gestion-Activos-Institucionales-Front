...
function SearchableSelect({ options, value, onChange, placeholder, loading, disabled }) {
 ...
 className="w-full px-3 py-2 pr-8 text-sm border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-2 focus:ring-green-500 outline-none disabled:bg-gray-50 disabled:text-gray-400"
 />
 <ChevronDown
 size={14}
 ...
 className={`w-full px-3 py-2 text-left text-sm transition-colors hover:bg-green-50 hover:text-green-700 dark:text-green-400 ${String(opt.value) === String(value) ? 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 dark:text-green-300 font-semibold' : 'text-gray-700 dark:text-gray-300 '}`}
 >
 {opt.label}
 </button>
 ))
...
export function CargaMasivaRowModal({ row, onSave, onClose }) {
 ...
 <Package size={18} className="text-blue-700 dark:text-blue-400 dark:text-blue-300" />
 </div>
 <div>
 <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 ">Editar Fila (Carga Masiva)</h2>
 <p className="text-xs text-gray-500 dark:text-gray-400 ">Serie: {formData.num_serie || 'Sin serie'}</p>
 </div>
 </div>
 <button onClick={onClose} className="p-2 text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-600 hover:text-gray-700 dark:text-gray-300 rounded-full transition-colors">
 <X size={20} />
 </button>
 </div>

 {/* TABS */}
 <div className="px-6 pt-3 flex gap-1 border-b border-gray-100 dark:border-gray-800 shrink-0">
 <button type="button" onClick={() => setActiveTab('generales')}
 className={`px-4 py-2 text-sm font-semibold rounded-t-lg transition-colors border-b-2 -mb-px ${activeTab === 'generales' ? 'border-green-600 text-green-700 dark:text-green-400 dark:text-green-300 bg-green-50 dark:bg-green-900/20' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 '}`}>
 Datos Generales
 </button>
 <button type="button" onClick={() => setActiveTab('specs')}
 className={`px-4 py-2 text-sm font-semibold rounded-t-lg transition-colors border-b-2 -mb-px flex items-center gap-1.5 ${activeTab === 'specs' ? 'border-blue-600 text-blue-700 dark:text-blue-400 dark:text-blue-300 bg-blue-50 dark:bg-blue-900/20' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 '}`}>
 ...
 <div className="bg-gray-50 dark:bg-gray-900 p-4 border border-gray-100 dark:border-gray-800 rounded-lg">
 ...
 <button type="button" onClick={() => setIsAddingModelo(!isAddingModelo)} className="text-xs text-green-600 dark:text-green-400 font-semibold hover:text-green-700 dark:text-green-400 flex items-center gap-1">
 {isAddingModelo ? 'Seleccionar existente' : <><Plus size={12}/> Crear nuevo</>}
 </button>
 </div>

 {isAddingModelo ? (
 <div className="space-y-4 fade-in bg-white dark:bg-gray-800 p-4 rounded border border-green-100 dark:border-green-800/50 shadow-sm">
 {row.invalidModelo && (
 <div className="p-3 bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 dark:text-red-300 rounded-lg text-xs font-semibold mb-2">
 ...
 <div className="p-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-gray-400">
 <Monitor size={16} />
 </div>
 ...
 />
 <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1">Si el monitor no existe, asegúrate de cargarlo también en el Excel antes que el CPU.</p>
 </div>
 </div>
 </div>
