        ...
        {!isCreating && !editingReporte ? (
          <div className="space-y-4">
            <div className="flex justify-between items-center mb-4">
              <h4 className="font-semibold text-slate-700 text-lg">Bitácora de Seguimiento</h4>
              {!readOnly && (
            ...
            {isLoading ? (
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-8 text-center shadow-sm">
                ...
                <h5 className="text-lg font-bold text-slate-700 mb-2">Sin seguimiento registrado</h5>
              ...
              <div className="space-y-4">
                {reportes.map(rep => (
                  <div key={rep.id_reporte_garantia} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden hover:shadow-md transition-shadow">
                    <div className="bg-slate-100/50 border-b border-slate-100 px-4 py-3 flex justify-between items-center">
                      <div className="flex items-center gap-2">
                        <EstatusIcon estatus={rep.estatus} size={18} />
                        <span className="font-bold text-slate-800">{rep.estatus}</span>
                      </div>
                      {!readOnly && (
                          <div className="flex items-center gap-2">
                            <button type="button" onClick={() => handleEditClick(rep)} className="p-1.5 text-slate-400 hover:text-[#006341] rounded bg-white border border-slate-200 shadow-sm transition-colors" title="Editar nota">
                              <Edit size={14} />
                            </button>
                            <button type="button" onClick={() => setReporteToDelete(rep)} className="p-1.5 text-slate-400 hover:text-red-600 rounded bg-white border border-slate-200 shadow-sm transition-colors" title="Eliminar nota">
                              <Trash2 size={14} />
                            </button>
                          </div>
                      )}
                    ...
                    <div className={`p-4 grid grid-cols-1 ${['Resuelto / Entregado', 'Listo para recoger'].includes(rep.estatus) ? 'md:grid-cols-2' : ''} gap-4`}>
                      <div>
                        ...
                        <p className="text-sm text-slate-700 whitespace-pre-wrap">{rep.descripcion_falla}</p>
                      </div>
                      {['Resuelto / Entregado', 'Listo para recoger'].includes(rep.estatus) && (
                        <div>
                          ...
                          {rep.resolucion ? (
                            <p className="text-sm text-slate-700 whitespace-pre-wrap">{rep.resolucion}</p>
                          ) : (
                    ...
                    <div className="bg-slate-50 px-4 py-2 border-t border-slate-100 flex flex-wrap gap-x-6 gap-y-2 text-xs text-slate-500">
                      <div className="flex items-center gap-1.5">
                        <Calendar size={13} /> Registrado: <span className="font-semibold text-slate-700">{new Date(Number(rep.fecha_reporte) || rep.fecha_reporte).toLocaleString('es-MX', { dateStyle: 'medium', timeStyle: 'short' })}</span>
                      </div>
                      ...
                      <div className="flex items-center gap-1.5 ml-auto">
                        Por: <span className="font-semibold text-slate-700">{rep.usuarioRegistra?.nombre_completo || 'Sistema'}{rep.usuarioRegistra?.matricula ? ` (${rep.usuarioRegistra.matricula})` : ''}</span>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="p-5 space-y-4">
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-1">Estatus de la Nota</label>
                <select 
                  value={form.estatus}
                  onChange={e => setForm(p => ({ ...p, estatus: e.target.value }))}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#006341] focus:border-[#006341] outline-none transition-all"
                >
              ...
              <div>
                ...
                <textarea 
                  ...
                  rows={4}
                  className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-[#006341] focus:border-[#006341] outline-none transition-all resize-none"
                />
              </div>
              {['Resuelto / Entregado', 'Listo para recoger'].includes(form.estatus) && (
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-1">Resolución / Diagnóstico del Proveedor</label>
                  <textarea 
      ...
      {reporteToDelete && ReactDOM.createPortal(
        <div className="fixed inset-0 z-[10000] flex items-center justify-center p-4 bg-black/50">
          <div className="bg-white rounded-xl shadow-xl p-6 max-w-sm w-full">
            ...
            <h4 className="text-center font-bold text-lg text-slate-800 mb-2">¿Eliminar nota?</h4>
