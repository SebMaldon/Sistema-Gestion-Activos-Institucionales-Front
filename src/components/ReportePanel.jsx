...
function StatCard({ label, value, color = '#006341', bg = '#F0FDF4', icon: Icon }) {
 return (
 <div className="flex items-center gap-2.5 p-3 rounded-xl border bg-white dark:bg-gray-800 shadow-sm" style={{ borderColor: color + '30', backgroundColor: bg }}>
 {Icon && (
...
function BarRow({ label, count, total, color = '#006341' }) {
 ...
 <span className="font-bold text-gray-900 dark:text-gray-100 shrink-0 mt-0.5">{count} <span className="text-gray-400 font-normal text-[10px]">({pct}%)</span></span>
 </div>
...
export default function ReportePanel({ serverFilter, activeTab, onClose }) {
 ...
 >
 <div 
 className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-5xl flex flex-col overflow-hidden zoom-in h-[85vh] border border-teal-800/10"
 ...
 <div className="absolute top-0 right-0 w-64 h-64 bg-white dark:bg-gray-800 opacity-5 rounded-full blur-3xl -translate-y-1/2 translate-x-1/3 pointer-events-none" />
 ...
 {loading && (
 <div className="absolute inset-0 flex flex-col items-center justify-center bg-white dark:bg-gray-800 z-20">
 <Loader2 size={36} className="animate-spin text-teal-600 dark:text-teal-400 mb-4" />
 <p className="text-sm font-bold text-gray-700 dark:text-gray-300 ">Calculando reporte de unidades...</p>
 </div>
 )}

 {error && (
 <div className="p-6 m-6 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 dark:border-red-800/50 rounded-xl text-center text-red-600 dark:text-red-400 font-medium">
 Error: {error}
 </div>
 )}
 ...
 <div className="flex items-center gap-2 px-4 py-2 bg-teal-50 dark:bg-teal-900/20 border border-teal-100 dark:border-teal-800/50 rounded-xl text-teal-800 dark:text-teal-300 w-full sm:w-auto">
 <BarChart2 size={16} />
 ...
 className="w-full pl-9 pr-4 py-2 border border-gray-200 dark:border-gray-700 rounded-xl text-sm outline-none focus:border-teal-500 focus:ring-1 focus:ring-teal-500 bg-white dark:bg-gray-800 shadow-sm"
 />
 </div>
 </div>
 ...
 className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl hover:border-teal-400 hover:shadow-md cursor-pointer transition-all gap-4"
 >
 <div className="flex-1">
 <div className="flex items-center gap-2 mb-1.5">
 <h3 className="font-bold text-gray-900 dark:text-gray-100 text-sm sm:text-base">{u.descripcion}</h3>
 <span className="text-[10px] bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 px-2 py-0.5 rounded-md font-mono">{u.clave}</span>
 </div>
 
 <div className="flex flex-wrap items-center gap-2 mt-2">
 <DevicePill icon={Cpu} count={u.pcs} label="PCs" colorCls="bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 dark:text-blue-300 border border-blue-100 dark:border-blue-800/50" />
 <DevicePill icon={Monitor} count={u.laptops} label="Laptops" colorCls="bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-800/50" />
 <DevicePill icon={Printer} count={u.impresoras} label="Impresoras" colorCls="bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 border border-purple-100" />
 <DevicePill icon={Network} count={u.switches} label="Switches" colorCls="bg-cyan-50 dark:bg-cyan-900/20 text-cyan-700 dark:text-cyan-400 border border-cyan-100" />
 <DevicePill icon={Smartphone} count={u.telefonosIP} label="Tel. IP" colorCls="bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 border border-emerald-100 dark:border-emerald-800/50" />
 <DevicePill icon={Phone} count={u.telefonosNormal} label="Tel. Analógico" colorCls="bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 border border-orange-100" />
 </div>
 </div>

 <div className="flex items-center gap-6 shrink-0">
 {u.inconvenientes > 0 && (
 <div className="flex items-center gap-1.5 text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 px-3 py-1.5 rounded-lg text-xs font-bold border border-red-100 dark:border-red-800/50">
 <AlertTriangle size={14} />
 {u.inconvenientes} Inconvenientes
 </div>
 )}
 <div className="text-right flex flex-col items-end">
 <p className="text-2xl font-black text-teal-800 dark:text-teal-300 leading-none">{u.total}</p>
 <p className="text-[10px] font-bold text-teal-600 dark:text-teal-400/70 uppercase tracking-widest mt-1">Dispositivos</p>
 </div>
 ...
 <div className="flex-1 overflow-y-auto custom-scrollbar p-5 sm:p-6 bg-white dark:bg-gray-800 fade-in space-y-6 pb-8">
 ...
 <div>
 <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-1.5 border-b border-gray-100 dark:border-gray-800 pb-2">
 <Cpu size={14} className="text-teal-600 dark:text-teal-400" /> Dispositivos y sus Estatus
 </h4>
 <div className="space-y-3 max-h-80 overflow-y-auto pr-2 custom-scrollbar">
 {Object.entries(detailStats.byTipoDetalle)
 .sort((a, b) => b[1].total - a[1].total)
 .map(([tipo, data]) => (
 <div key={tipo} className="bg-gray-50/80 dark:bg-gray-900/20 border border-gray-200 dark:border-gray-700 rounded-xl p-3 shadow-sm">
 <div className="flex justify-between items-center mb-2">
 <span className="font-bold text-gray-800 dark:text-gray-200 text-sm">{tipo}</span>
 <span className="text-[10px] font-black text-teal-800 dark:text-teal-300 bg-teal-100 dark:bg-teal-900/20 px-2 py-0.5 rounded-md border border-teal-200 dark:border-teal-800/50">{data.total} total</span>
 </div>
 ...
 <div>
 <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-1.5 border-b border-gray-100 dark:border-gray-800 pb-2">
 <CheckCircle size={14} className="text-teal-600 dark:text-teal-400" /> Estatus Operativo
 </h4>
 <div className="space-y-3 p-4 bg-gray-50 dark:bg-gray-900 rounded-xl border border-gray-100 dark:border-gray-800 shadow-inner">
 ...
 <div>
 <div className="flex flex-col gap-2 mb-3 border-b border-gray-100 dark:border-gray-800 pb-2">
 <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
 <MapPin size={14} className="text-teal-600 dark:text-teal-400" /> Distribución por Ubicación
 </h4>
 ...
 className={`text-[10px] px-2 py-1 rounded-md border transition-colors ${isSelected ? 'bg-teal-600 text-white border-teal-700 font-bold shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 '}`}
 >
 {t}
 </button>
 );
 ...
 className="w-full pl-8 pr-3 py-1.5 border border-gray-200 dark:border-gray-700 rounded-lg text-xs outline-none focus:border-teal-500 bg-white dark:bg-gray-800 "
 />
 </div>
 </div>
 ...
 <div key={ubicacion} className="p-3 bg-gray-50/80 dark:bg-gray-900/20 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm">
 <div className="flex justify-between items-center mb-2">
 <p className="text-sm font-bold text-gray-800 dark:text-gray-200 ">{ubicacion}</p>
 <span className="text-[10px] font-black text-teal-700 dark:text-teal-400 bg-teal-100 dark:bg-teal-900/20 px-2 py-0.5 rounded-md border border-teal-200 dark:border-teal-800/50">{stats.total} total</span>
 </div>
 ...
 return (
 <span key={t} className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border ${isHighlighted ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 dark:text-blue-300 border-blue-100 dark:border-blue-800/50' : 'bg-gray-100 dark:bg-gray-800 text-gray-400 border-gray-200 dark:border-gray-700 opacity-60'}`}>
 {t}: {count}
 </span>
 );
 })}
 ...
 <div>
 <h4 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest mb-3 flex items-center gap-1.5 border-b border-gray-100 dark:border-gray-800 pb-2">
 <Shield size={14} className="text-teal-600 dark:text-teal-400" /> Resumen de Garantías
 </h4>
