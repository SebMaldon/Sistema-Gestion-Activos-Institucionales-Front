...
function TabCatalogo({ atributos, loading, showToast }) {
 ...
 const inputCls = "w-full px-3 py-2 text-sm border border-gray-200 dark:border-gray-700 rounded-lg focus:ring-1 focus:ring-purple-500 outline-none";
 ...
 <label className="block text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Nombre *</label>
 ...
 <div>
 <label className="block text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Tipo de Dato</label>
 ...
 <div>
 <label className="block text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Unidad Medida</label>
 ...
 <div className="lg:col-span-1">
 <label className="block text-[10px] font-semibold text-gray-500 dark:text-gray-400 uppercase mb-1">Descripción</label>
 ...
 {editingId && (
 <button onClick={handleCancel} className="px-4 py-2 rounded-lg text-sm font-semibold text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 ">Cancelar</button>
 )}
 ...
 {/* Lista */}
 <div>
 <h3 className="text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">Atributos Existentes</h3>
 {loading ? (
 ...
 {atributos.map(a => (
 <tr key={a.id_atributo} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 transition-colors">
 ...
 <td className="px-4 py-2 text-xs text-gray-500 dark:text-gray-400 ">{TIPO_OPCIONES.find(o => o.value === a.tipo_valor)?.label}</td>
 <td className="px-4 py-2 text-xs text-gray-500 dark:text-gray-400 ">{a.unidad_medida || '—'}</td>
...
function TabAsignacion({ tipos, atributos, showToast }) {
 ...
 <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-2">Selecciona un Tipo de Dispositivo</label>
 <select 
 value={selectedTipo} 
 onChange={e => setSelectedTipo(e.target.value)}
 className="w-full sm:w-1/2 px-3 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-purple-500 outline-none bg-white dark:bg-gray-800 "
 >
 ...
 {/* Asignados */}
 <div className="border border-green-200 dark:border-green-800/50 dark:border-green-800/50 rounded-xl overflow-hidden flex flex-col">
 <div className="bg-green-50 dark:bg-green-900/20 px-4 py-3 border-b border-green-200 dark:border-green-800/50 dark:border-green-800/50">
 <h3 className="text-xs font-bold text-green-800 dark:text-green-300 uppercase">Atributos Sugeridos</h3>
 <p className="text-[10px] text-green-600 dark:text-green-400">Aparecerán automáticamente en la Ficha Técnica de este tipo.</p>
 </div>
 <div className="p-3 space-y-2 flex-1 bg-white dark:bg-gray-800 ">
 {isLoading ? (
 ...
 asignados.map(a => (
 <div key={a.id_atributo} className="flex items-center justify-between p-2 rounded-lg bg-green-50 dark:bg-green-900/20 border border-green-100 dark:border-green-800/50">
 <div>
 <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 ">{a.atributo?.nombre_atributo}</p>
 <p className="text-[10px] text-gray-500 dark:text-gray-400 ">{TIPO_OPCIONES.find(o => o.value === a.atributo?.tipo_valor)?.label}</p>
 </div>
 <button onClick={() => handleRemove(a.id_atributo)} className="text-red-400 hover:text-red-600 p-1 bg-white dark:bg-gray-800 rounded shadow-sm border border-red-100 dark:border-red-800/50" title="Quitar">
 <Unlink size={14} />
 </button>
 </div>
 ))
 ...
 <h3 className="text-xs font-bold text-gray-700 dark:text-gray-300 uppercase">Otros Atributos</h3>
 <p className="text-[10px] text-gray-500 dark:text-gray-400 ">Haz clic en el '+' para agregarlo a la lista de sugeridos.</p>
 </div>
 <div className="p-3 space-y-2 flex-1 bg-white dark:bg-gray-800 max-h-80 overflow-y-auto">
 {disponibles.length === 0 ? (
 <p className="text-xs text-gray-400 text-center py-4">No hay más atributos disponibles en el catálogo.</p>
 ) : (
 disponibles.map(a => (
 <div key={a.id_atributo} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 border border-transparent hover:border-gray-100 dark:border-gray-800 group transition-colors">
 <div>
 <p className="text-sm font-medium text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:text-gray-100 ">{a.nombre_atributo}</p>
 </div>
 <button onClick={() => handleAssign(a.id_atributo)} disabled={isAssigning} className="text-purple-600 dark:text-purple-400 hover:text-white hover:bg-purple-600 p-1 border border-purple-200 dark:border-purple-800/50 rounded transition-colors disabled:opacity-50" title="Asignar">
 <Plus size={14} />
 </button>
 </div>
 ))
