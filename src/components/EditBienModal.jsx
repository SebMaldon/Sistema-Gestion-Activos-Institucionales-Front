...
function ModeloCatalogModal({ onClose, onSelectModelo, modeloActual, catalogos }) {
 ...
 tab === t.id ? 'border-green-600 text-green-700 dark:text-green-400 dark:text-green-300 bg-white dark:bg-gray-800 ' : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 '
 }`}>
 ...
 hasPendingChange ? 'border-amber-300 bg-amber-50 dark:bg-amber-900/20' : 'border-green-200 dark:border-green-800/50 dark:border-green-800/50 bg-green-50 dark:bg-green-900/20'
 }`}>
 <div className="flex items-center justify-between mb-2">
 <p className={`text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 ${
 hasPendingChange ? 'text-amber-700 dark:text-amber-400 dark:text-amber-300' : 'text-green-700 dark:text-green-400 dark:text-green-300'
 }`}>
 {hasPendingChange
 ? <>○ Pendiente confirmar</>
...
export function Modal({ onClose, title, subtitle, children, footer, wide = false, small = false }) {
 ...
 <div className={`relative bg-white dark:bg-gray-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden w-full max-h-[calc(100dvh-2rem)] sm:max-h-[calc(100vh-4rem)] ${
...
export function EditBienModal({ isOpen, onClose, asset, catalogos, mode = 'edit', refetch }) {
 ...
 <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md overflow-hidden flex flex-col" onClick={e => e.stopPropagation()}>
 <div className="px-6 py-4 border-b border-gray-100 dark:border-gray-800 flex items-center justify-between bg-orange-50 dark:bg-orange-900/20 shrink-0">
 ...
 <ul className="list-disc pl-5 mb-4 text-sm text-red-600 dark:text-red-400 font-semibold space-y-1">
 ...
 <div key={d.id_bien} className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg p-2 text-xs flex flex-col gap-0.5">
 <p className="font-semibold text-gray-800 dark:text-gray-200 ">{d.modelo?.descrip_disp || 'Equipo'}</p>
 <p className="text-gray-500 dark:text-gray-400 font-mono">S/N: {d.num_serie || 'N/D'} | INV: {d.num_inv || 'N/D'}</p>
 {d.ubicacion?.nombre && <p className="text-gray-500 dark:text-gray-400 text-[10px]">Ubicación: {d.ubicacion.nombre}</p>}
 </div>
 ))}
 </div>
 </div>
 ...
 <button onClick={() => setInconveniencesWarning(null)} className="text-xs text-gray-400 hover:text-gray-600 dark:text-gray-400 underline">
 Cancelar Guardado
 </button>
 </div>
 </div>
 ...
 className="px-4 py-2.5 rounded-xl text-sm font-semibold border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 transition-colors">
 Cancelar
 </button>
 <button
 ...
 forcePieza ? 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 cursor-not-allowed' : 'bg-white dark:bg-gray-800 '
 } ${formErrors.id_unidad_medida && !forcePieza ? 'border-red-400' : 'border-gray-200 dark:border-gray-700 '}`}
 >
 ...
 {/* Número de Serie */}
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Número de Serie</label>
 <input
 type="text"
 ...
 className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500 ${
 formErrors.num_serie ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : 'border-gray-200 dark:border-gray-700 '
 }`}
 />
 ...
 <div className="flex gap-2">
 <div
 className="flex-1 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm bg-gray-50 dark:bg-gray-900 text-gray-700 dark:text-gray-300 cursor-pointer hover:border-green-400 transition-colors flex items-center justify-between min-w-0"
 ...
 type="button"
 onClick={() => setShowCatalogModal(true)}
 className="px-3 py-2 rounded-lg border border-green-200 dark:border-green-800/50 dark:border-green-800/50 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 dark:text-green-300 text-xs font-semibold hover:bg-green-100 transition-colors flex items-center gap-1.5 whitespace-nowrap flex-shrink-0"
 >
 ...
 className="w-9 h-9 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 hover:text-red-400 transition-colors flex items-center justify-center flex-shrink-0"
 title="Quitar modelo"
 >
 <X size={13} />
 </button>
 ...
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Número de Inventario
{modalForm !== 'create' && modalForm?.inconvenientes?.includes('Sin número de inventario') && (<span className="ml-2 inline-flex items-center gap-1 text-red-600 dark:text-red-400 text-[10px]" title="Este equipo requiere número de inventario"><AlertTriangle size={12} className="animate-pulse" /> Faltante</span>)}
</label>
 <input
 type="text"
 value={form.num_inv}
 ...
 className={`w-full border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500 ${
 formErrors.num_inv ? 'border-red-400 bg-red-50 dark:bg-red-900/20' : 'border-gray-200 dark:border-gray-700 '
 }`}
 />
 ...
 {/* Estatus Operativo */}
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Estatus Operativo</label>
 <select
 value={form.estatus_operativo}
 onChange={(e) => setForm((f) => ({ ...f, estatus_operativo: e.target.value }))}
 className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500 bg-white dark:bg-gray-800 "
 >
 ...
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Cantidad{esSerie && <span className="ml-2 text-xs text-blue-500 font-normal"> (forzado a 1)</span>}</label>
 <input
 type="number"
 min="1"
 ...
 disabled={esSerie}
 className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500"
 />
 </div>
 ...
 {/* Unidad Física */}
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Unidad Física</label>
 <SearchableSelect
 ...
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Área / Ubicación Física</label>
 {isAddingUbicacion ? (
 <div className="flex gap-2">
 <input
 type="text"
 ...
 className="flex-1 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500"
 autoFocus
 />
 ...
 className="px-3 py-2 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 rounded-lg text-sm font-semibold hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 transition-colors">
 <X size={15} />
 </button>
 </div>
 ) : (
 ...
 className="px-3 py-2 border border-gray-200 dark:border-gray-700 text-gray-500 dark:text-gray-400 flex items-center justify-center rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 transition-colors">
 <Plus size={15} />
 </button>
 </div>
 )}
 ...
 {/* Usuario Resguardo */}
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Usuario en Resguardo</label>
 <SearchableSelect
 ...
 {/* Fecha Adquisición */}
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Fecha de Adquisición</label>
 <input
 type="date"
 ...
 className="w-full flex items-center justify-between px-4 py-3 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 dark:text-blue-300 text-xs font-semibold uppercase tracking-wide"
 >
 ...
 className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-blue-400"
 />
 </div>
 ))}
 </div>
 ...
 {showTI && (
 <div className="border-t border-blue-100 dark:border-blue-800/50 p-4 bg-white dark:bg-gray-800 ">
 <div className="flex items-center justify-between mb-3">
 <p className="text-xs font-semibold text-blue-700 dark:text-blue-400 dark:text-blue-300 uppercase tracking-wide flex items-center gap-1.5">
 ...
 <div className="border-t border-blue-100 dark:border-blue-800/50 p-4">
 <div className="flex items-center justify-between mb-3">
 <p className="text-xs font-semibold text-blue-700 dark:text-blue-400 dark:text-blue-300 uppercase tracking-wide flex items-center gap-1.5">
 ...
 <p className="text-xs text-gray-400 italic text-center py-3 bg-gray-50 dark:bg-gray-900 rounded-lg border border-dashed border-gray-200 dark:border-gray-700 ">Sin cuentas — agrega una con el botón</p>
 )}
 <div className="space-y-3">
 {cuentasList.map((c, idx) => (
 <div key={c.id_cuenta ?? `new-${idx}`} className="rounded-lg border border-blue-100 dark:border-blue-800/50 overflow-hidden">
 {/* Header de cuenta */}
 <div className="flex items-center justify-between px-3 py-2 bg-blue-50 dark:bg-blue-900/20">
 <span className="text-xs font-semibold text-blue-700 dark:text-blue-400 dark:text-blue-300">
 {c.cuenta_windows || `Cuenta ${idx + 1}`}
 </span>
 <div className="flex gap-1">
 <button type="button"
 onClick={() => setCuentasList(prev => prev.map((x, i) => i === idx ? { ...x, _editing: !x._editing } : x))}
 className="text-[10px] px-2 py-0.5 rounded bg-white dark:bg-gray-800 border border-blue-200 dark:border-blue-800/50 dark:border-blue-800/50 text-blue-600 dark:text-blue-400 hover:bg-blue-100 font-semibold"
 ...
 className="text-[10px] px-2 py-0.5 rounded bg-white dark:bg-gray-800 border border-red-200 dark:border-red-800/50 dark:border-red-800/50 text-red-500 hover:bg-red-50 font-semibold"
 >Eliminar</button>
 </div>
 </div>
 {/* Campos editables */}
 {c._editing && (
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3 bg-white dark:bg-gray-800 ">
 {[
 ...
 placeholder={placeholder}
 className="w-full border border-gray-200 dark:border-gray-700 rounded px-2 py-1.5 text-xs focus:outline-none focus:ring-1 focus:ring-blue-400"
 />
 </div>
 ))}
 </div>
 ...
 {!c._editing && (
 <div className="px-3 py-2 bg-white dark:bg-gray-800 grid grid-cols-2 gap-1">
 {c.cuenta_windows && <span className="text-[10px] text-gray-600 dark:text-gray-400 "><span className="font-semibold">Win:</span> {c.cuenta_windows}</span>}
 {c.correo && <span className="text-[10px] text-gray-600 dark:text-gray-400 "><span className="font-semibold">Correo:</span> {c.correo}</span>}
 {c.tipo_user && <span className="text-[10px] text-gray-600 dark:text-gray-400 "><span className="font-semibold">Tipo:</span> {c.tipo_user}</span>}
 </div>
 )}
 </div>
 ))}
 ...
 {deviceMode === 'OTHER' && (
 <div className="rounded-xl border border-purple-200 dark:border-purple-800/50 overflow-hidden mt-4">
 <div className="px-4 py-3 bg-purple-50 dark:bg-purple-900/20 flex items-center justify-between">
 <div className="flex items-center gap-2">
 <Tag size={14} className="text-purple-700 dark:text-purple-400" />
 ...
 className="text-xs flex items-center gap-1 text-purple-600 dark:text-purple-400 hover:text-purple-800 bg-purple-100 dark:bg-purple-900/20 hover:bg-purple-200 px-2 py-1 rounded transition-colors"
 >
 ...
 <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden mb-2">
 <div className="bg-gray-50 dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 p-4 flex items-center justify-between">
 ...
 <div className={`absolute left-0.5 top-0.5 bg-white dark:bg-gray-800 w-4 h-4 rounded-full transition-transform ${garantiaForm.show ? 'translate-x-5' : 'translate-x-0'}`} />
 </div>
 </label>
 </div>
 {garantiaForm.show && (
 <div className="p-4 grid grid-cols-1 sm:grid-cols-3 gap-4 bg-green-50 dark:bg-green-900/20">
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Fecha Inicio</label>
 <input
 type="date"
 className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500 bg-white dark:bg-gray-800 "
 ...
 <div>
 <label className="block text-xs font-semibold text-gray-700 dark:text-gray-300 mb-1">Fecha Fin</label>
 <input
 type="date"
 className="w-full border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500 bg-white dark:bg-gray-800 "
 ...
 <button type="button" onClick={() => handleAutoCalcGarantia(1)} className="text-[10px] px-1.5 py-0.5 bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 dark:text-green-300 border border-green-200 dark:border-green-800/50 dark:border-green-800/50 rounded hover:bg-green-200">+1 Año</button>
 <button type="button" onClick={() => handleAutoCalcGarantia(2)} className="text-[10px] px-1.5 py-0.5 bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 dark:text-green-300 border border-green-200 dark:border-green-800/50 dark:border-green-800/50 rounded hover:bg-green-200">+2 Años</button>
 <button type="button" onClick={() => handleAutoCalcGarantia(3)} className="text-[10px] px-1.5 py-0.5 bg-green-100 dark:bg-green-900/20 text-green-700 dark:text-green-400 dark:text-green-300 border border-green-200 dark:border-green-800/50 dark:border-green-800/50 rounded hover:bg-green-200">+3 Años</button>
 </div>
 </div>
 <div>
 ...
 <div className="flex gap-2">
 <select
 className="flex-1 border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-green-500 bg-white dark:bg-gray-800 "
