...
export default function DetalleIncidenciaWrapperModal({ id_incidencia, onClose }) {
 ...
 <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-10 flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
 <Loader2 size={32} className="animate-spin text-indigo-600 dark:text-indigo-400 mb-4" />
 <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 ">Cargando detalles de la incidencia...</p>
 </div>
 </div>
 );
 }
 ...
 <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-gray-900/70 fade-in" onClick={onClose}>
 <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl p-10 flex flex-col items-center justify-center" onClick={e => e.stopPropagation()}>
 <p className="text-sm font-semibold text-red-600 dark:text-red-400 mb-4">Error al cargar la incidencia.</p>
 <button onClick={onClose} className="px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-600 text-sm font-bold">Cerrar</button>
 </div>
 </div>
 );
 }
