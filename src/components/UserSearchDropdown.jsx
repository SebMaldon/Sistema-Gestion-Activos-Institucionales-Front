...
export default function UserSearchDropdown({ usuarios, value, onChange }) {
 ...
 {isOpen && (
 <div className="absolute z-50 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-xl max-h-60 overflow-y-auto fade-in">
 {filteredUsers.length === 0 ? (
 <div className="p-3 text-sm text-gray-500 dark:text-gray-400 text-center">No se encontraron usuarios</div>
 ) : (
 <ul className="py-1">
 {filteredUsers.map(u => (
 <li
 ...
 >
 <span className="text-sm font-semibold text-gray-800 dark:text-gray-200 ">{u.nombre_completo}</span>
 {u.matricula && <span className="text-xs text-gray-500 dark:text-gray-400 font-mono">Matrícula: {u.matricula}</span>}
 </li>
 ))}
 </ul>
 )}
