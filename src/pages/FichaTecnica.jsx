...
function QRPlaceholder({ value }) {
 ...
 return (
 <div className="relative inline-block p-3 bg-white dark:bg-gray-800 border-2 border-gray-200 dark:border-gray-700 rounded-xl">
 {/* Corner markers */}
 ...
 <p className="text-center text-xs text-gray-500 dark:text-gray-400 font-mono mt-1">{value.slice(0, 20)}</p>
 </div>
 );
}
...
function InfoRow({ icon: Icon, label, value }) {
 ...
 <div className="w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0" className="bg-green-50 dark:bg-green-900/20">
 <Icon size={14} style={{ color: '#006341' }} />
 </div>
 <div className="flex-1 min-w-0">
 <p className="text-xs text-gray-400 font-medium">{label}</p>
 <p className="text-sm font-semibold text-gray-800 dark:text-gray-200 mt-0.5 break-words">{value || '—'}</p>
 </div>
 </div>
 );
}
...
export default function FichaTecnica() {
 ...
 {/* Backdrop */}
 <div
 className={`fixed inset-0 bg-black/50 dark:bg-black/70 z-50 transition-opacity ${isFichaOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
 onClick={closeFicha}
 />

 {/* Modal */}
 <div className={`fixed right-0 top-0 h-full w-full md:max-w-2xl bg-white dark:bg-gray-800 z-[60] shadow-2xl flex flex-col transition-transform duration-300 ${isFichaOpen ? 'translate-x-0' : 'translate-x-full'}`}>
 {/* Modal Header */}
 <div className="px-6 py-5 border-b border-gray-100 dark:border-gray-800 flex items-start justify-between" style={{ background: 'linear-gradient(135deg, #006341, #004d32)' }}>
 <div>
 ...
 <button onClick={closeFicha} className="w-9 h-9 rounded-lg bg-white dark:bg-gray-800/10 dark:bg-gray-800/20 flex items-center justify-center text-white hover:bg-white dark:bg-gray-800/20 dark:hover:bg-gray-700/20 transition-colors mt-1">
 <X size={18} />
 </button>
 </div>
 ...
 className="mt-2 w-full text-xs font-medium py-2 rounded-lg border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 transition-colors flex items-center justify-center gap-1.5">
