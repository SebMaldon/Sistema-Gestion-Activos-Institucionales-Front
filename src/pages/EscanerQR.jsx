...
function InfoField({ label, value, icon, mono = false, alert = null }) {
 ...
 {alert && (
 <span className="inline-flex items-center gap-1 text-[10px] font-bold text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800/50 dark:border-red-800/50 px-1.5 py-0.5 rounded">
...
function SoftwareTable({ programas }) {
 ...
 className="pl-8 pr-3 py-1.5 text-xs border border-gray-200 dark:border-gray-700 rounded-lg focus:outline-none focus:ring-1 focus:ring-green-500 w-48 bg-white dark:bg-gray-800 "
 />
 </div>
 </div>
 <div className="max-h-[300px] overflow-y-auto custom-scrollbar bg-white dark:bg-gray-800 ">
 <table className="w-full text-left border-collapse min-w-[600px]">
 <thead className="bg-gray-50/80 dark:bg-gray-900/20 sticky top-0 backdrop-blur-sm shadow-sm">
 <tr>
 <th className="px-4 py-2.5 text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700 ">Programa</th>
 <th className="px-4 py-2.5 text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700 w-32">Versión</th>
 <th className="px-4 py-2.5 text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider border-b border-gray-200 dark:border-gray-700 w-48">Editor</th>
 </tr>
 </thead>
 <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
 {filtered.map((p, i) => (
 <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 transition-colors">
 <td className="px-4 py-2 text-xs font-medium text-gray-800 dark:text-gray-200 ">{p.programa || '—'}</td>
 <td className="px-4 py-2 text-xs text-gray-500 dark:text-gray-400 font-mono">{p.version || '—'}</td>
 <td className="px-4 py-2 text-xs text-gray-500 dark:text-gray-400 truncate max-w-[12rem]">{p.editor || '—'}</td>
 </tr>
 ))}
 {filtered.length === 0 && (
 <tr>
...
export default function EscanerQR() {
 ...
 <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 ">Escáner QR</h1>
 <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Escanea el código QR o ingresa cualquier identificador del bien</p>
 </div>
 ...
 <div className="xl:col-span-4 bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden flex flex-col shrink-0">
 ...
 <div className="relative">
 <div className="absolute inset-0 flex items-center" aria-hidden><div className="w-full border-t border-gray-200 dark:border-gray-700 " /></div>
 <div className="relative flex justify-center">
 <span className="bg-white dark:bg-gray-800 px-3 text-xs text-gray-400">o ingresa un identificador</span>
 </div>
 </div>
 ...
 className="w-full pl-8 pr-3 py-2.5 text-sm rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 text-gray-800 dark:text-gray-200 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
 />
 </div>
 <button
 ...
 {isFetching && (
 <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-14 flex flex-col items-center gap-3">
 <Loader2 size={32} className="animate-spin text-green-600 dark:text-green-400" />
 <p className="text-sm text-gray-500 dark:text-gray-400 font-medium">Buscando activo...</p>
 </div>
 )}

 {!isFetching && (!foundAssets || foundAssets.length === 0) && activeHash && (
 <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm p-10 text-center fade-in">
 <p className="text-gray-400 text-sm">No se encontró ningún activo con ese identificador.</p>
 </div>
 )}

 {!isFetching && (!foundAssets || foundAssets.length === 0) && !activeHash && (
 <div className="hidden xl:flex bg-gray-50 dark:bg-gray-900 rounded-2xl border border-dashed border-gray-300 dark:border-gray-600 p-14 flex-col items-center justify-center text-center h-full">
 <Package size={48} className="text-gray-300 mb-4" />
 <p className="text-gray-500 dark:text-gray-400 font-medium">Esperando escaneo</p>
 ...
 <button onClick={handleReset} className="text-sm font-semibold text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:text-gray-200 transition-colors px-4 py-2">
 Cancelar búsqueda
 </button>
 </div>
 }
 ...
 <div className="pr-8">
 <p className="font-bold text-gray-900 dark:text-gray-100 text-sm sm:text-base leading-snug group-hover:text-green-800 transition-colors">
 {asset.equipo}
 </p>
 ...
 <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-2 gap-x-4 mt-auto pt-3 border-t border-gray-100 dark:border-gray-800/80">
 <div className="flex items-center gap-1.5 text-[11px] sm:text-xs">
 <span className="text-gray-400"><Hash size={13} /></span>
 <span className="text-gray-500 dark:text-gray-400 font-medium">S/N:</span>
 <span className="font-semibold text-gray-800 dark:text-gray-200 truncate">{asset.numSerie || 'N/A'}</span>
 </div>
 <div className="flex items-center gap-1.5 text-[11px] sm:text-xs">
 <span className="text-gray-400"><Tag size={13} /></span>
 <span className="text-gray-500 dark:text-gray-400 font-medium">Inv:</span>
 <span className="font-semibold text-gray-800 dark:text-gray-200 truncate">{asset.numInv || 'N/A'}</span>
 </div>
 {asset.especificacionTI?.dir_ip && (
 <div className="flex items-center gap-1.5 text-xs sm:col-span-2 mt-1">
 <span className="inline-flex items-center gap-1.5 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 dark:text-blue-300 px-2.5 py-1 rounded-md border border-blue-100 dark:border-blue-800/50 font-bold tracking-wide shadow-sm">
 ...
 <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200 dark:border-gray-700 shadow-sm overflow-hidden fade-in flex flex-col xl:h-full min-h-0">
 
 {/* ── Encabezado del Bien ── */}
 <div className="p-5 flex items-start sm:items-center justify-between flex-col sm:flex-row gap-4 bg-gradient-to-br from-green-50/80 to-emerald-50 dark:to-emerald-900/20/80 dark:from-green-900/40 dark:to-emerald-900/40 border-b border-green-100 dark:border-green-800/50">
 ...
 <p className="font-bold text-gray-900 dark:text-gray-100 text-lg leading-tight truncate">{foundAsset.equipo}</p>
 <p className="text-xs text-gray-500 dark:text-gray-400 truncate mt-0.5">{foundAsset.categoria?.nombre_categoria || 'Bien de Inventario'}</p>
 </div>
 </div>
 ...
 <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[11px] font-bold bg-red-100 dark:bg-red-900/20 text-red-700 dark:text-red-400 dark:text-red-300 animate-pulse border border-red-200 dark:border-red-800/50 dark:border-red-800/50">
 ...
 {/* ── Pestañas Ficha ── */}
 <div className="flex gap-1 border-b border-gray-200 dark:border-gray-700 px-4 pt-3 bg-gray-50/30 dark:bg-gray-900/20 overflow-x-auto overflow-y-hidden shrink-0 scrollbar-hide">
 {[
 ...
 fichaTabs === t.key
 ? 'border-green-600 text-green-700 dark:text-green-400 dark:text-green-300 bg-white dark:bg-gray-800 '
 : 'border-transparent text-gray-500 dark:text-gray-400 hover:text-gray-700 dark:text-gray-300 hover:bg-gray-100/50 dark:bg-gray-800/50 dark:hover:bg-gray-600/50'
 }`}
 >
 {t.label}
 </button>
 ...
 {/* Notas */}
 <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden mt-6">
 <div className="bg-gray-50 dark:bg-gray-900 px-4 py-2.5 flex items-center justify-between border-b border-gray-100 dark:border-gray-800 ">
 <div className="flex items-center gap-2">
 ...
 className="w-full text-sm border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none resize-none"
 rows={2}
 />
 ...
 <span className="text-xs font-semibold text-gray-800 dark:text-gray-200 ">{foundAsset.equipoAsignado.equipo?.modelo?.descrip_disp || 'Equipo genérico'}</span>
 <span className="text-[10px] text-gray-500 dark:text-gray-400 font-mono mt-1">ID: {foundAsset.equipoAsignado.equipo?.id_bien || 'N/A'}</span>
 <span className="text-[10px] text-gray-500 dark:text-gray-400 font-mono">S/N: {foundAsset.equipoAsignado.equipo?.num_serie || 'S/N'} | INV: {foundAsset.equipoAsignado.equipo?.num_inv || 'S/N'}</span>
 </div>
 </div>
 </div>
 )}
 ...
 {fichaMode === 'OTHER' && (
 <div className="rounded-xl border border-purple-200 dark:border-purple-800/50 overflow-hidden">
 <div className="bg-purple-50 dark:bg-purple-900/20 px-4 py-2.5 flex items-center gap-2 border-b border-purple-100">
 <Tag size={15} className="text-purple-700 dark:text-purple-400" />
 ...
 foundAsset.garantias[0].estado_garantia === 'VIGENTE' ? 'bg-green-100 dark:bg-green-900/20 text-green-800 dark:text-green-300' :
 foundAsset.garantias[0].estado_garantia === 'VENCIDA' ? 'bg-red-100 dark:bg-red-900/20 text-red-800 dark:text-red-300' :
 ...
 {/* ── Footer de Acciones ── */}
 <div className="p-4 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-800 flex flex-wrap gap-3 shrink-0">
 <button onClick={() => { if (foundAssets?.length > 1) { setSelectedIndex(null); } else { handleReset(); } }} className="flex-1 py-2 rounded-xl text-sm font-semibold text-gray-600 dark:text-gray-400 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors shadow-sm">
 ← {foundAssets?.length > 1 ? 'Volver a la lista' : 'Escanear otro'}
 </button>
 {puedeEditar && (
 <button onClick={() => setEditModalOpen(true)} className="flex-1 py-2 flex justify-center items-center gap-1.5 rounded-xl text-sm font-semibold text-green-700 dark:text-green-400 dark:text-green-300 bg-white dark:bg-gray-800 border border-green-200 dark:border-green-800/50 dark:border-green-800/50 hover:bg-green-50 transition-colors shadow-sm">
 <Edit size={14} /> Editar
 </button>
 )}
 {puedeEliminar && (
 <button onClick={handleDelete} className="flex-1 py-2 flex justify-center items-center gap-1.5 rounded-xl text-sm font-semibold text-red-600 dark:text-red-400 bg-white dark:bg-gray-800 border border-red-200 dark:border-red-800/50 dark:border-red-800/50 hover:bg-red-50 transition-colors shadow-sm">
 ...
 ¿Estás seguro de que deseas eliminar permanentemente el bien <strong className="text-gray-900 dark:text-gray-100 ">{foundAsset.equipo}</strong>?
 </p>
 ...
 <p><span className="text-gray-400">ID:</span> <span className="font-mono text-gray-700 dark:text-gray-300 ">{foundAsset.id_bien || foundAsset.id}</span></p>
 <p><span className="text-gray-400">Serie:</span> <span className="font-mono text-gray-700 dark:text-gray-300 ">{foundAsset.numSerie || 'S/N'}</span></p>
 <p><span className="text-gray-400">Inv:</span> <span className="font-mono text-gray-700 dark:text-gray-300 ">{foundAsset.numInv || 'S/N'}</span></p>
 </div>
