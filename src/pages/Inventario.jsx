                ...
                <div className="flex items-center gap-3 p-4 rounded-xl bg-gradient-to-br from-green-50 to-emerald-50 dark:from-green-900/40 dark:to-emerald-900/40 border border-green-100 dark:border-green-800/50">
                  ...
                  {[ROL_ADMIN, ROL_MAESTRO].includes(idRol) && (fichaMode === 'PC' || fichaMode === 'LAPTOP') && (
                    <button
                      ...
                      disabled={!activeFicha.programasPC?.some(p => p.programa?.includes('SGH') || p.programa?.includes('SGHI'))}
                      onClick={async () => {
                        try {
                      ...
                      title={!activeFicha.programasPC?.some(p => p.programa?.includes('SGH') || p.programa?.includes('SGHI')) ? "Agente SGH no instalado" : "Forzar escaneo de este equipo"}
                      className={`px-3 py-1.5 text-xs font-semibold rounded-lg shadow-sm flex items-center gap-1.5 transition-colors ${!activeFicha.programasPC?.some(p => p.programa?.includes('SGH') || p.programa?.includes('SGHI')) ? 'bg-gray-100 dark:bg-gray-800 text-gray-400 border border-gray-200 dark:border-gray-700 cursor-not-allowed' : 'bg-white dark:bg-gray-800 border border-amber-200 dark:border-amber-800/50 dark:border-amber-800/50 text-amber-700 dark:text-amber-400 dark:text-amber-300 hover:bg-amber-50'}`}
                    >
                      <RefreshCw size={12} />
