...
const ACTION_CONFIG = {
 CREACION: { icon: FilePlus, bg: 'bg-green-100 dark:bg-green-900/30', color: 'text-green-600 dark:text-green-400', hex: '#16a34a', label: 'Creación' },
 EDICION: { icon: Edit, bg: 'bg-orange-50 dark:bg-orange-900/30', color: 'text-orange-600 dark:text-orange-400', hex: '#ea580c', label: 'Edición' },
 ELIMINACION: { icon: Trash2, bg: 'bg-red-100 dark:bg-red-900/30', color: 'text-red-600 dark:text-red-400', hex: '#dc2626', label: 'Eliminación' },
 LECTURA: { icon: Eye, bg: 'bg-gray-100 dark:bg-gray-800', color: 'text-gray-500 dark:text-gray-400', hex: '#6b7280', label: 'Lectura' },
 LOGIN: { icon: ShieldCheck, bg: 'bg-sky-100 dark:bg-sky-900/30', color: 'text-sky-700 dark:text-sky-400', hex: '#0369a1', label: 'Inicio de Sesión' },
};
...
function DetalleJSONModal({ isOpen, onClose, log, catalogs, onVerBien, onVerIncidencia }) {
 ...
 >
 <div 
 className="bg-white dark:bg-gray-800 w-full max-w-3xl rounded-2xl shadow-2xl flex flex-col max-h-[90vh] overflow-hidden animate-slide-up"
 ...
 <button onClick={onClose} className="text-gray-400 hover:text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-600 p-2 rounded-xl transition-colors">
 <X size={20} />
 </button>
 </div>
 
 <div className="p-6 overflow-y-auto bg-gray-50/30 dark:bg-gray-900/20 flex-1">
 <div className="flex flex-wrap gap-4 mb-6">
 <div className="bg-white dark:bg-gray-800 px-4 py-2.5 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm flex-1 min-w-[200px]">
 ...
 <span 
 className="text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 ml-1 cursor-pointer underline decoration-indigo-300 decoration-2 underline-offset-2" 
 ...
 <span className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: ACTION_CONFIG[log.accion]?.hex || '#9ca3af' }} />
 <p className="text-sm font-bold text-gray-800 dark:text-gray-200 uppercase tracking-tight">{ACTION_CONFIG[log.accion]?.label || log.accion}</p>
 </div>
 </div>
 </div>
 ...
 <tr>
 <th className="px-4 py-3 text-xs font-black text-gray-600 dark:text-gray-400 uppercase tracking-wider w-1/3">Propiedad</th>
 <th className="px-4 py-3 text-xs font-black text-red-600 dark:text-red-400 uppercase tracking-wider w-1/3 border-l border-gray-200 dark:border-gray-700 ">Valor Anterior</th>
 <th className="px-4 py-3 text-xs font-black text-green-600 dark:text-green-400 uppercase tracking-wider w-1/3 border-l border-gray-200 dark:border-gray-700 ">Valor Nuevo</th>
 </tr>
 </thead>
 ...
 if (u) return <span className="text-indigo-600 dark:text-indigo-400 font-bold">{u.descripcion || u.nombre} <span className="text-gray-400 font-normal text-[9px]">({val})</span></span>;
 }
 ...
 if (i) return <span className="text-indigo-600 dark:text-indigo-400 font-bold">{i.descripcion} <span className="text-gray-400 font-normal text-[9px]">({val})</span></span>;
 }
 if (field === 'id_segmento') {
 const s = catalogs.catSegmentos?.find(s => String(s.id_segmento) === valStr);
 if (s) return <span className="text-indigo-600 dark:text-indigo-400 font-bold">{s.nombre || s.no_ref} <span className="text-gray-400 font-normal text-[9px]">({val})</span></span>;
 }
 if (field === 'clave_modelo') {
 const m = catalogs.catModelos?.find(m => String(m.clave_modelo) === valStr);
 if (m) return <span className="text-indigo-600 dark:text-indigo-400 font-bold">{m.descrip_disp} <span className="text-gray-400 font-normal text-[9px]">({val})</span></span>;
 }
 if (field === 'id_categoria') {
 const c = catalogs.catCategoriasActivo?.find(c => String(c.id_categoria) === valStr);
 if (c) return <span className="text-indigo-600 dark:text-indigo-400 font-bold">{c.nombre_categoria} <span className="text-gray-400 font-normal text-[9px]">({val})</span></span>;
 }
 if (field === 'id_unidad_medida') {
 const m = catalogs.catUnidadesMedida?.find(m => String(m.id_unidad_medida) === valStr);
 if (m) return <span className="text-indigo-600 dark:text-indigo-400 font-bold">{m.nombre_unidad} <span className="text-gray-400 font-normal text-[9px]">({val})</span></span>;
 }
 if (field === 'id_proveedor') {
 const p = catalogs.proveedores?.find(p => String(p.id_proveedor) === valStr);
 if (p) return <span className="text-indigo-600 dark:text-indigo-400 font-bold">{p.nombre_proveedor} <span className="text-gray-400 font-normal text-[9px]">({val})</span></span>;
 }
 }
 ...
 return (
 <tr key={col} className="hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 transition-colors">
 <td className="px-4 py-3 font-semibold text-gray-700 dark:text-gray-300 bg-gray-50/30 dark:bg-gray-900/20 border-r border-gray-100 dark:border-gray-800 ">
 ...
 <td className="px-4 py-3 font-mono text-[11px] text-red-700 dark:text-red-400 dark:text-red-300 bg-red-50 dark:bg-red-900/20 break-words border-r border-gray-100 dark:border-gray-800 ">
 {formatVal(col, vAnterior)}
 </td>
 <td className="px-4 py-3 font-mono text-[11px] text-green-700 dark:text-green-400 dark:text-green-300 bg-green-50 dark:bg-green-900/20 break-words">
 ...
 <div className="w-20 h-20 rounded-full bg-green-50 dark:bg-green-900/20 flex items-center justify-center mb-5 border border-green-100 dark:border-green-800/50 shadow-sm">
 <ShieldCheck size={40} className="text-green-600 dark:text-green-400" />
 </div>
 <h4 className="text-xl font-bold text-gray-900 dark:text-gray-100 ">Inicio de Sesión Exitoso</h4>
 ...
 {parsed?.info && (
 <div className="mt-4 p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 rounded-xl max-w-md w-full text-left">
 ...
 <p className="text-sm text-indigo-600 dark:text-indigo-400 font-medium">{parsed.info}</p>
 </div>
 )}
 <div className="mt-6 flex items-center gap-2 px-4 py-2 bg-gray-100 dark:bg-gray-800 rounded-full text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
...
export default function Auditoria() {
 ...
 <div>
 <h1 className="text-xl sm:text-2xl font-bold text-gray-900 dark:text-gray-100 ">Bitácora de Auditoría</h1>
 <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Registro global de movimientos del sistema — Solo lectura</p>
 </div>
 <div className="flex items-center gap-2">

 <div className="flex items-center gap-2 px-4 py-2 rounded-xl border border-amber-200 dark:border-amber-800/50 dark:border-amber-800/50 bg-amber-50 dark:bg-amber-900/20">
 ...
 <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 flex-shrink-0">
 <div className="p-4 border-b border-gray-50 dark:border-gray-800 bg-gray-50/30 dark:bg-gray-900/20">
 <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
 <div className="flex items-center gap-2.5 px-3.5 py-2 bg-indigo-50 dark:bg-indigo-900/20 rounded-xl text-xs sm:text-sm border border-indigo-100 dark:border-indigo-800/50 font-bold text-indigo-700 dark:text-indigo-400 shadow-sm w-fit">
 <Activity size={16} className="text-indigo-500" />
 <span className="whitespace-nowrap">Eventos Totales: <span className="text-gray-900 dark:text-gray-100 ml-0.5">{totalCount}</span></span>
 </div>
 ...
 ? 'bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 text-indigo-600 dark:text-indigo-400 shadow-sm'
 : 'bg-white dark:bg-gray-800 border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/50 dark:hover:bg-gray-700 '
 }`}
 >
 <Filter size={16} />
 ...
 {showFilters && (
 <div className="mt-4 p-5 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700/80 rounded-2xl shadow-inner animate-in fade-in slide-in-from-top-1 duration-250">
 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
 {/* Grupo 1: Operación */}
 <div className="space-y-3">
 <h3 className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider border-b border-indigo-100 dark:border-indigo-800/50 pb-1">
 Detalles de la Operación
 </h3>
 <div>
 <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-0.5">Acción Realizada</label>
 <MultiSelect
 ...
 <div>
 <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-0.5">Módulo Afectado</label>
 <MultiSelect
 ...
 <div>
 <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-0.5">Usuario</label>
 <MultiSelect
 ...
 <div>
 <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-0.5">Plataforma de Origen</label>
 <select 
 value={filterOrigen} 
 onChange={e => { setFilterOrigen(e.target.value); setCursor(null); setCursors([]); }}
 className="w-full px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg text-xs focus:ring-2 focus:ring-indigo-500 outline-none transition-all"
 >
 ...
 <div>
 <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-0.5">Desde</label>
 <input 
 type="date"
 ...
 <div>
 <label className="block text-[10px] font-bold text-gray-500 dark:text-gray-400 uppercase mb-0.5">Hasta</label>
 <input 
 type="date"
 ...
 <div className="mt-4 pt-3 border-t border-gray-200 dark:border-gray-700/80 flex items-center justify-between">
 ...
 {/* Tabla desktop */}
 <div className="hidden md:block flex-1 min-h-0 bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-auto">
 {isLoading ? (
 ...
 <thead className="bg-gray-50 dark:bg-gray-900 sticky top-0 z-10 border-b border-gray-100 dark:border-gray-800 ">
 <tr>
 <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[15%]">Fecha / Hora</th>
 <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[25%]">Usuario</th>
 <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[10%]">Origen</th>
 <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[18%]">Acción</th>
 <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider w-[18%]">Módulo Afectado</th>
 <th className="px-6 py-4 text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider text-center w-[14%]">Detalles</th>
 </tr>
 </thead>
 ...
 return (
 <tr key={log.id_bitacora} className="hover:bg-gray-50 dark:hover:bg-gray-800/50/80 dark:hover:bg-gray-700/80 transition-colors group animate-slide-down">
 <td className="px-6 py-4 whitespace-nowrap">
 <span className="font-mono text-[11px] font-semibold text-gray-500 dark:text-gray-400 ">{formatDateTime(log.fecha_movimiento)}</span>
 </td>
 <td className="px-6 py-4 truncate">
 <p className="font-bold text-gray-800 dark:text-gray-200 truncate" title={log.usuario.nombre_completo}>{log.usuario.nombre_completo}</p>
 <p className="text-gray-400 font-mono text-[10px] mt-0.5">{log.usuario.matricula}</p>
 </td>
 <td className="px-6 py-4">
 {log.origen ? (
 <span className="text-[10px] font-black uppercase text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-2 py-1 rounded-md border border-indigo-100 dark:border-indigo-800/50">{log.origen}</span>
 ) : (
 ...
 <div className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 shadow-sm ${conf.bg}`}>
 <Icon size={14} className={conf.color} />
 </div>
 <span className={`text-[11px] font-black uppercase tracking-wider break-words ${conf.color}`}>{conf.label}</span>
 </div>
 </td>
 <td className="px-6 py-4 min-w-0">
 <div className="flex items-center gap-2 min-w-0">
 <span className="font-bold text-[11px] text-gray-700 dark:text-gray-300 bg-gray-100 dark:bg-gray-800 px-2.5 py-1 rounded-md border border-gray-200 dark:border-gray-700 uppercase tracking-wide flex-shrink-0">
 {log.tabla_afectada}
 </span>
 {isValidId(log.registro_afectado) && (log.tabla_afectada === 'Bienes' || log.tabla_afectada === 'Especificaciones_TI') ? (
 <span 
 className="text-[11px] font-mono font-bold text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 truncate cursor-pointer underline decoration-indigo-300 decoration-2 underline-offset-2" 
 title="Ver Ficha Técnica"
 ...
 className="inline-flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg border border-indigo-100 dark:border-indigo-800/50 bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 text-[11px] uppercase tracking-wide font-black hover:bg-indigo-600 hover:text-white hover:border-indigo-600 transition-all shadow-sm active:scale-95"
 >
 ...
 const Icon = conf.icon;
 return (
 <div key={log.id_bitacora} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm p-4 relative overflow-hidden animate-slide-down">
 <div className="flex items-center gap-3 mb-3">
 <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${conf.bg}`}>
 <Icon size={18} className={conf.color} />
 </div>
 <div className="flex-1 min-w-0">
 <div className="flex items-center justify-between">
 <div className="flex items-center gap-2">
 <p className={`text-sm font-bold truncate ${conf.color}`}>{conf.label}</p>
 {log.origen && <span className="text-[9px] font-black uppercase text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/20 px-1.5 py-0.5 rounded-sm border border-indigo-100 dark:border-indigo-800/50">{log.origen}</span>}
 </div>
 <span className="text-[10px] font-mono text-gray-400">{formatDateTime(log.fecha_movimiento)}</span>
 </div>
 <p className="text-xs text-gray-600 dark:text-gray-400 font-semibold mt-0.5 truncate">
 ...
 <span 
 className="font-mono text-indigo-500 hover:text-indigo-700 dark:text-indigo-400 truncate cursor-pointer underline decoration-indigo-300 decoration-2 underline-offset-2" 
 onClick={() => setVisualModalBienId(log.registro_afectado)}
 title="Ver Ficha Técnica"
 >
 #{log.registro_afectado}
 ...
 <p className="text-xs text-gray-600 dark:text-gray-400 "><span className="text-gray-400 font-medium">Por:</span> <span className="font-semibold text-gray-800 dark:text-gray-200 ">{log.usuario.nombre_completo}</span></p>
 {log.detalles_movimiento && log.detalles_movimiento !== 'null' && (
 <button
 onClick={() => setModalLog(log)}
 className="mt-3 w-full flex items-center justify-center gap-2 px-3 py-2 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 text-indigo-700 dark:text-indigo-400 rounded-lg text-xs font-bold hover:bg-indigo-100 transition-colors"
 >
 ...
 {/* ─── Paginación ─────────────────────────────────────────── */}
 <div className="p-3 border-t border-gray-100 dark:border-gray-800 flex flex-col gap-2 bg-gray-50 dark:bg-gray-900 flex-shrink-0 rounded-b-2xl">
 {/* Info total */}
