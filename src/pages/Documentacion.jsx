...
export default function Documentacion() {
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
      {/* Navbar */}
      <nav className="bg-[#00472e] text-white shadow-lg px-6 py-4 flex items-center gap-4 sticky top-0 z-50">
        <Link to="/" className="p-2 hover:bg-white/10 rounded-xl transition-colors" title="Volver al inicio">
      ...
      <div className="flex-1 w-full max-w-[1400px] mx-auto p-4 md:p-8 flex flex-col gap-10">
        ...
        {/* ═══ CABECERA ═══ */}
        <section className="bg-white p-8 md:p-12 rounded-2xl shadow-sm border border-gray-200 relative overflow-hidden">
        ...
        <section className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-gray-200">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b pb-4">
            <div>
              <h3 className="text-2xl font-bold text-gray-800">Diagrama de Casos de Uso</h3>
              <p className="text-sm text-gray-500 mt-1">Interacciones funcionales por actor — Diagrama UML completo</p>
            </div>
            <span className="px-4 py-1.5 bg-green-100 text-green-800 font-semibold rounded-full text-sm">
              UML / Funcional
            </span>
          </div>
          <div className="w-full overflow-x-auto p-4 bg-gray-50/50 rounded-xl border border-gray-100 min-h-[600px]">
        ...
        <section className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-gray-200">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b pb-4">
            <div>
              <h3 className="text-2xl font-bold text-gray-800">Arquitectura del Sistema</h3>
              <p className="text-sm text-gray-500 mt-1">Componentes de alto nivel y flujo de comunicación</p>
            </div>
            <span className="px-4 py-1.5 bg-blue-100 text-blue-800 font-semibold rounded-full text-sm">
              Infraestructura
            </span>
          </div>
          <div className="w-full overflow-x-auto p-4 bg-gray-50/50 rounded-xl border border-gray-100 min-h-[350px] flex items-center justify-center">
        ...
        <section className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-gray-200">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b pb-4">
            <div>
              <h3 className="text-2xl font-bold text-gray-800">Ciclo de Vida de un Activo</h3>
              <p className="text-sm text-gray-500 mt-1">Estados y transiciones de un bien en el sistema</p>
            </div>
            <span className="px-4 py-1.5 bg-amber-100 text-amber-800 font-semibold rounded-full text-sm">
              State Machine
            </span>
          </div>
          <div className="w-full overflow-x-auto p-4 bg-gray-50/50 rounded-xl border border-gray-100 min-h-[300px] flex items-center justify-center">
        ...
        <section className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-gray-200">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4 border-b pb-4">
            <div>
              <h3 className="text-2xl font-bold text-gray-800">Flujo de Aprobación de Cambios</h3>
              <p className="text-sm text-gray-500 mt-1">Proceso de solicitud, revisión y aprobación de modificaciones</p>
            </div>
            <span className="px-4 py-1.5 bg-orange-100 text-orange-800 font-semibold rounded-full text-sm">
              Workflow
            </span>
          </div>
          <div className="w-full overflow-x-auto p-4 bg-gray-50/50 rounded-xl border border-gray-100 min-h-[300px] flex items-center justify-center">
        ...
        <section>
          <h3 className="text-2xl font-bold text-gray-800 mb-6">Roles del Sistema</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
            {[
              ...
              <div key={r.role} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 border-t-4 hover:shadow-md transition-shadow flex flex-col" style={{ borderTopColor: r.color }}>
                <div className="flex items-center gap-3 mb-4">
                  <div className="p-3 rounded-xl" style={{ backgroundColor: r.bg, color: r.color }}>
                    <r.icon className="w-6 h-6" />
                  </div>
                  <div>
                    <h4 className="text-lg font-bold text-gray-800">{r.role}</h4>
        ...
        {/* ═══ CATÁLOGO DE MÓDULOS ═══ */}
        <section>
          <h3 className="text-2xl font-bold text-gray-800 mb-2">Módulos del Sistema</h3>
          <p className="text-gray-500 text-sm mb-6">Descripción funcional detallada de cada módulo y sus capacidades por rol</p>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {modulos.map(m => (
              <div key={m.nombre} className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 hover:shadow-md transition-all group">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="text-base font-bold text-gray-800 mb-1">{m.nombre}</h4>
                    <p className="text-sm text-gray-600 leading-relaxed mb-3">{m.descripcion}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {m.roles.map((r, i) => (
                        <span key={i} className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-[10px] font-semibold uppercase tracking-wide">
                          {r}
                        </span>
                      ))}
                    </div>
        ...
        {/* ═══ TABLA DE ENTIDADES ═══ */}
        <section className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-gray-200">
          <h3 className="text-2xl font-bold text-gray-800 mb-2">Entidades Principales del Sistema</h3>
          <p className="text-sm text-gray-500 mb-6">Resumen de las tablas y entidades que conforman el modelo de datos</p>
          <div className="overflow-x-auto rounded-xl border border-gray-200">
            <table className="w-full text-sm text-left">
              <thead className="bg-gray-50 border-b border-gray-200">
                <tr>
                  <th className="px-5 py-3.5 text-xs font-bold text-gray-600 uppercase tracking-wider">Entidad</th>
                  <th className="px-5 py-3.5 text-xs font-bold text-gray-600 uppercase tracking-wider">Tabla BD</th>
                  <th className="px-5 py-3.5 text-xs font-bold text-gray-600 uppercase tracking-wider">Descripción</th>
                  <th className="px-5 py-3.5 text-xs font-bold text-gray-600 uppercase tracking-wider">Módulo</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100">
                {[
                  ...
                  <tr key={i} className="hover:bg-gray-50/80 transition-colors">
                    <td className="px-5 py-3 font-semibold text-gray-800">{entidad}</td>
                    <td className="px-5 py-3 font-mono text-xs text-blue-600 bg-blue-50/30">{tabla}</td>
                    <td className="px-5 py-3 text-gray-600">{desc}</td>
                    <td className="px-5 py-3">
                      <span className="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-[10px] font-bold uppercase">{modulo}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        {/* ═══ TECNOLOGÍAS ═══ */}
        <section className="bg-white p-6 md:p-10 rounded-2xl shadow-sm border border-gray-200">
          <h3 className="text-2xl font-bold text-gray-800 mb-6">Stack Tecnológico</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={s.cat} className="bg-gray-50 p-5 rounded-xl border border-gray-100">
                <h4 className="text-sm font-bold text-gray-700 uppercase tracking-wider mb-3 pb-2 border-b border-gray-200">{s.cat}</h4>
